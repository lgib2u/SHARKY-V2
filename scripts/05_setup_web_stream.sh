...
echo "Installing Flask + Picamera2 on the Pi..."
remote_sudo "apt update && apt install -y python3-flask python3-picamera2"
...
def snapshot():
    # Grab a single JPEG frame by waiting for next frame in stream
    start_camera_once()
    with output.condition:
...
if __name__ == '__main__':
    # Bind on all interfaces for access via AP or LAN
